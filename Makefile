...
format :; forge fmt
#This is Foundry’s default test mnemonic
#Enables step-by-step EVM execution tracing, Useful for debugging with tools like cast, forge test -vvvv, or trace viewers
#Forces Anvil to auto-mine a new block every 1 second
...
# using '''make deploy ARGS="--network sepolia" ''' to trigger the sepolia deployment
# set ACCOUNT at.env using Keystore account
