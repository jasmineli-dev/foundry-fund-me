...
contract DeployFunWithStorage is Script {
    function run() external returns (FunWithStorage) {
        ...
        printMapping(address(funWithStorage));
    ...
    function printMapping(address contractAddress) public view {
        bytes32 slot = keccak256(abi.encode(0, uint256(3)));
        bytes32 value = vm.load(contractAddress, slot);
        console.log("MyMap[0] storage location:");
        console.logBytes32(slot);
        console.log("MyMap[0] value:");
        console.logBytes32(value);
    }
